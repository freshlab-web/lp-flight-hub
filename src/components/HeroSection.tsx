  ...
  return (
    <section className="relative min-h-screen pt-20 overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-secondary via-secondary to-navy-light">
        ...
        <div className="absolute inset-0 bg-[url('data:image/svg+xml,%3Csvg%20width%3D%2260%22%20height%3D%2260%22%20viewBox%3D%220%200%2060%2060%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%3E%3Cg%20fill%3D%22none%22%20fill-rule%3D%22evenodd%22%3E%3Cg%20fill%3D%22%230057FF%22%20fill-opacity%3D%220.03%22%3E%3Cpath%20d%3D%22M36%2034v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6%2034v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6%204V0H4v4H0v2h4v4h2V6h4V4H6z%22%2F%3E%3C%2Fg%3E%3C%2Fg%3E%3C%2Fsvg%3E')] opacity-50" />
      </div>
      ...
      <div className="container relative mx-auto px-4 lg:px-8 py-12 lg:py-20">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center min-h-[calc(100vh-12rem)]">
          ...
          <div className="space-y-8 animate-fade-in-left" style={{ animationDelay: "0.2s" }}>
            <Badge 
              variant="outline" 
              className="px-4 py-2 border-primary/30 bg-primary/10 text-primary-foreground backdrop-blur-sm"
            >
            ...
            <div className="space-y-6">
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-black text-secondary-foreground leading-tight">
                Hardware profissional para{" "}
                <span className="gradient-text">operações críticas</span>
              </h1>
            ...
            <div className="flex flex-wrap gap-4 pt-4">
              {["DJI Matrice", "DJI Dock", "Câmeras Térmicas", "LiDAR"].map((item) => (
                <div
                  key={item}
                  className="flex items-center gap-2 px-4 py-2 rounded-full bg-secondary-foreground/5 border border-secondary-foreground/10"
                >
                  <div className="w-2 h-2 rounded-full bg-primary" />
                  <span className="text-sm font-medium text-secondary-foreground/80">{item}</span>
                </div>
              ))}
            </div>
          </div>
